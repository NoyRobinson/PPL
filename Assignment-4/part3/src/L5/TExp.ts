...
*/
import { chain, concat, flatten, indexOf, map, union, uniq } from "ramda";
...
const parseUnionTExp = (texps: Sexp[]): Result<TExp> => {
    if(isEmpty(texps)|| texps[0] !== "union" || texps.length === 1 || texps.length !== 3 ){
        return makeFailure("Union structure not defined well");
    }

    const comps = (texps.slice(1)).map(parseTExp);
    if (!allT(isOk, comps)){
        return makeFailure("Components are undefined");
    }

    const compCheck = comps.map((x)=> {
        if(isOk(x))
            return x.value;
        else
            return makeVoidTExp();
    ...
    const unionCheck = flatten((compCheck.filter(isUnionTExp)).map((x) => x.components));

    const parsedUnion = uniq(union(unionCheck, compCheck.filter((x) => !isUnionTExp(x)))).sort((te1, te2) => {
        if(te1.tag === te2.tag)
            return 0;
        else if(te2.tag < te1.tag)
            return 1;
        else 
            return -1;
    });

    if(parsedUnion.length === 1)
        return makeOk(parsedUnion[0]);

    return makeOk({tag: "UnionTExp", components: parsedUnion});
};
...
export const unparseTExp = (te: TExp): Result<string> => {
    ...
    const unparseUnion = (ut: TExp[]): Result<string> => {
        if(ut.length === 1 || ut.length === 0)
            return makeFailure("length is illegal");
        if(ut.length === 2) {
            const firstComp = unparseTExp(ut[0]);  
            const secondComp = unparseTExp(ut[1]);
            if(isOk(firstComp) && isOk(secondComp)){
                if(firstComp.value < secondComp.value)
                    return makeOk(`(union ${firstComp.value} ${secondComp.value})`);
                else
                    return makeOk(`(union ${secondComp.value} ${firstComp.value})`);
            }
        }
        else {
           const small = ut.sort((curr, prev) => {
                if(unparseTExp(curr) < unparseTExp(prev))
                    return -1;
                else
                    return 1;
           });
           const uPSmall = unparseTExp(small[0]);
           const uPrec = unparseUnion(ut.filter((v) => v !== small[0]));
           if(isOk(uPSmall) && isOk(uPrec))
               return makeOk(`(union ${uPSmall.value} ${uPrec.value})`);
        }
        return makeFailure("shouldn't happen");
    }
