// L5-typecheck
// ========================================================
import { equals, map, zipWith, reduce, uniq, union } from 'ramda';
import { isAppExp, isBoolExp, isDefineExp, isIfExp, isLetrecExp, isLetExp, isNumExp,
         isPrimOp, isProcExp, isProgram, isStrExp, isVarRef, parseL5Exp, unparse,
         AppExp, BoolExp, DefineExp, Exp, IfExp, LetrecExp, LetExp, NumExp,
         Parsed, PrimOp, ProcExp, Program, StrExp, parseL5Program, makeStrExp, makeProgram } from "./L5-ast";
import { applyTEnv, makeEmptyTEnv, makeExtendTEnv, TEnv } from "./TEnv";
import { isProcTExp, makeBoolTExp, makeNumTExp, makeProcTExp, makeStrTExp, makeVoidTExp,
         parseTE, unparseTExp,
         BoolTExp, NumTExp, StrTExp, TExp, VoidTExp, isUnionTExp, makeUnionTExp, ProcTExp, UnionTExp } from "./TExp";
import { isEmpty, allT, first, rest, NonEmptyList, List, isNonEmptyList } from '../shared/list';
import { Result, makeFailure, bind, makeOk, zipWithResult, isFailure, mapv, isOk } from '../shared/result';
...
const checkSubType = (subTe2: TExp, te2: TExp): boolean =>{
    if(isUnionTExp(te2) && !isUnionTExp(subTe2)){
       const afterFilter = te2.components.filter(x => checkSubType(subTe2, x));
       return !isEmpty(afterFilter);
    }

    else if(!isUnionTExp(te2) && isUnionTExp(subTe2)){
        return false;
    }

    else if(isUnionTExp(te2) && isUnionTExp(subTe2)){
        const afterFilter = subTe2.components.filter(x => checkSubType(te2, x));
        return afterFilter.length === subTe2.components.length;
    }

    return equals(subTe2, te2);
}

const checkParams = (param1: TExp[], param2: TExp[]): boolean => {
    if(isEmpty(param1))
        return true;
    
    if(checkSubType(param2[0], param1[0]))
        return checkParams(param1.slice(1), param2.slice(1));
    
    return false;
}

const checkSubProc = (tproc1: ProcTExp, tproc2: ProcTExp): Result<true> =>{
    if(tproc1.paramTEs.length !== tproc2.paramTEs.length)
        return makeFailure("Not the same length");

    if(!checkSubType(tproc1.returnTE, tproc2.returnTE))
        return makeFailure("tproc1's returnExp isn't contained in tproc2's returnExp");

    if(!checkParams(tproc1.paramTEs, tproc2.paramTEs))
        return makeFailure("∀i ∈ [1 . . . n1], p2,i !⊆ p1,i"); 
    
    return makeOk(true);
}

export const checkCompatibleType = (te1: TExp, te2: TExp, exp: Exp): Result<true> =>
    checkSubType(te1, te2) ? makeOk(true) :
    isProcTExp(te1) && isProcTExp(te2) ? checkSubProc(te1, te2) : 
...
export const makeUnion = (te1: TExp, te2: TExp): TExp => {
    if(isUnionTExp(te1) && !isUnionTExp(te2)){
        return unifier(union(te1.components, [te2]));
    }
 
    else if(!isUnionTExp(te1) && isUnionTExp(te2)){
        return unifier(union([te1], te2.components));
    }
 
    else if(isUnionTExp(te1) && isUnionTExp(te2)){
        return unifier(union(te1.components, te2.components));
    }

    if(isOk(checkCompatibleType(te1, te2, makeStrExp("")))){
        return te2;
    }

    if(isOk(checkCompatibleType(te2, te1, makeStrExp("")))){
        return te1;
    }

    return {tag: "UnionTExp", components: [te1, te2].sort((firstComp, secondComp) => {
        if(firstComp.tag > secondComp.tag)
            return 1;
        else
            return -1;
        })
...
const unifier = (texps : TExp[]): TExp => {
    if(!isEmpty(texps)){
        if(texps.length !== 1)
            return {tag: "UnionTExp", components: uniq(texps.sort((firstComp, secondComp) => {
                if(firstComp.tag > secondComp.tag)
                    return 1;
                else
                    return -1;
...
export const typeofIf = (ifExp: IfExp, tenv: TEnv): Result<TExp> => {
    ...
    if(isFailure(constraint2)){
        return bind(constraint1, (_c1: true) =>
            bind(thenTE, thenTe => bind(altTE, altTe => makeOk(makeUnion(thenTe, altTe)))));
    }
...
export const typeofDefine = (exp: DefineExp, tenv: TEnv): Result<VoidTExp> => {
    const constraints = bind(typeofExp(exp.val, tenv), (typeOfVal: TExp) => 
                            checkCompatibleType(exp.var.texp, typeOfVal, exp));
...
export const typeofProgram = (exp: Program, tenv: TEnv): Result<TExp> => {
    ...
    if(isDefineExp(exp.exps[0])) {
        if(isOk(typeofDefine(exp.exps[0], tenv)))
            return typeofProgram(makeProgram(exp.exps.slice(1)), makeExtendTEnv([exp.exps[0].var.var], [exp.exps[0].var.texp] ,tenv));
        else
            return makeFailure("Type of define is not defined well");
    }
    else {
        if(isOk(typeofExp(exp.exps[0], tenv)))
            return typeofProgram(makeProgram(exp.exps.slice(1)), tenv);
        else
            return makeFailure("Type of exp is not defined well");
    }
}

export const L5ProgType = (prog:string) : Result<string> =>{
    return bind(p(prog), (x) =>
        bind(parseL5Program(x), (e: Program) => {
            return bind(typeofExp(e, makeEmptyTEnv()), unparseTExp);
        }));
}