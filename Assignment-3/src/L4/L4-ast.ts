...
;;         |  ( bound? <var> )                / BoundExp(var: varRef) ## added for 2.1.1
;;         |  ( time <cexp> )                 / TimeExp(exp: cexp)  ## added for 2.2.1
...
export const makeTimeExp = (ce: CExp): TimeExp => ({tag: "TimeExp", exp: ce});
...
const parseBoundExp = (params: Sexp[]): Result<BoundExp> =>
    (params.length !== 1) ? makeFailure('length is not 1') :
    (allT(isDefineExp, params) || allT(isCompoundExp, params)) ? makeFailure('shouldnt parse bound') :
    bind(parseL4CExp(params[0]), (val: CExp) =>
        isVarRef(val) ? makeOk(makeBoundExp(val)) :
            makeFailure('not varRef'));
    
const parseTimeExp = (params: Sexp[]): Result<TimeExp> =>
    (isEmpty<Sexp>(params)) ? makeFailure("empty params") :
    (params.length !== 1) ? makeFailure("length isn't 1") :
    bind(parseL4CExp(params[0]), (val: CExp) =>
        makeOk(makeTimeExp(val)));
...
const unparseBoundExp = (b: BoundExp): string =>
    `(bound? ${unparse(b.var)})`;

const unparseTimeExp = (t: TimeExp): string =>
    `(time ${unparse(t.exp)})`;
