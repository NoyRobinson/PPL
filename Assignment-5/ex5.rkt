...
(define append$
  (lambda (lst1 lst2 cont)
    ...
    (append$ (cdr lst1) lst2
         (lambda(res_cdr_lst1) (cont (cons (car lst1) res_cdr_lst1))
         )
     )
    )
  )
)
     ; if lst1 is empty, return continuation lst2.
     ; append (cdr lst1) and y, and the function which gets the result of the append and calculates
     ; (car lst1) and the result from append and sends it to the continuation.
...
(define equal-trees$
  (lambda (tree1 tree2 succ fail)
    (cond
      ...
      ((or (and (empty? tree1) (not (empty? tree2)))
          ...
          (equal-trees$
               ...
               (lambda (res-car-tree)
                  (equal-trees$
                       (cdr tree1) (cdr tree2)
                       (lambda (res-cdr-tree)
                           (succ (cons res-car-tree res-cdr-tree))
                       )
                       fail
                   )
                )
